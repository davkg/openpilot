"""
Lane centering, ported from StarPilot (firestar5683/StarPilot).

Biases the model's desired curvature toward the geometric center of the detected ego lane before
clip_curvature, so the correction stays subject to the normal curvature/jerk limits and the torque
and NNLC lateral controllers are fed unchanged.

"Confidence-gated": when the model's own path is confident (low position.yStd) but disagrees with
lane center by a lot, the correction is faded out on the assumption that the model is leaving
center deliberately. LaneCenteringE2EAuthority scales how much deference the model gets.
"""
