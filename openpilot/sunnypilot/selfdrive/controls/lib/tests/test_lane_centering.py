...
class TestLaneCentering(OpenpilotTestCase):
  ...
  def test_driver_override_resets_correction(self):
    model = _model(model_y=-0.3)
    controller, steady = _converge(model)
    assert steady != 0.0

    # a driver nudge drops the correction immediately rather than fading it
    assert _update(controller, model, driver_override=True) == 0.0
    assert controller._correction == 0.0

  def test_driver_override_recovers_when_released(self):
    model = _model(model_y=-0.3)
    controller, _ = _converge(model)
    _update(controller, model, driver_override=True)
    out = 0.0
    for _ in range(300):
      out = _update(controller, model)
    assert out != 0.0


class TestLaneCenteringVisualDirection(OpenpilotTestCase):
  def _direction(self, model, **kwargs):
    kwargs.setdefault("enabled", True)
    kwargs.setdefault("lat_active", True)
    return get_lane_centering_visual_direction(model, _V_EGO, kwargs.pop("offset", 0.0),
                                               kwargs.pop("authority", 1.0), kwargs.pop("enabled"),
                                               kwargs.pop("lat_active"), kwargs.pop("pause_on_signal", False),
                                               kwargs.pop("turn_signal_active", False),
                                               kwargs.pop("applied_correction", None))

  def test_direction_follows_side_of_error(self):
    # model path left of lane center -> correction pulls right (+y is right)
    assert self._direction(_model(model_y=-0.3)) == 1
    assert self._direction(_model(model_y=0.3)) == -1

  def test_no_direction_when_centered(self):
    assert self._direction(_model(model_y=0.0)) == 0

  def test_no_direction_when_disabled_or_inactive(self):
    model = _model(model_y=-0.3)
    assert self._direction(model, enabled=False) == 0
    assert self._direction(model, lat_active=False) == 0

  def test_no_direction_while_paused_on_turn_signal(self):
    model = _model(model_y=-0.3)
    assert self._direction(model, pause_on_signal=True, turn_signal_active=True) == 0

  def test_applied_correction_overrides_raw_sign(self):
    # the smoothed output still lags on the far side of a sign flip; the tint should follow it
    model = _model(model_y=-0.3)
    assert self._direction(model, applied_correction=-1e-3) == -1

  def test_zero_applied_correction_falls_back_to_raw(self):
    model = _model(model_y=-0.3)
    assert self._direction(model, applied_correction=0.0) == 1
